		...
		if($stat_type == 'month'){
			...
			for($i=$first_day;$i<=$last_day;$i+=86400){
				...
				foreach($operator_info as $operator_id=>$row){
					$operator_stat[$i]['stat'][$operator_id] = $this->where("operator_id = %d AND create_year = %d AND create_month = '%s' AND create_day = '%d' AND order_type = %d AND status = 1",array($operator_id,date('Y'),date('m'),date('d',$i),self::DEPOSIT_ORDER_TYPE))->sum('amount');
				}
